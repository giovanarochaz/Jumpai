...
// --- ESTILOS DO CENÁRIO (ESTRELAS E FUNDO) ---
...
const animacaoEstrelas = keyframes`
...
// --- ESTILOS DOS CORPOS CELESTES ---

const moverPlaneta = keyframes`
...
// --- ESTILOS DO ASTRONAUTA E EFEITOS DE COLISÃO ---

const flutuar = keyframes`
  0% { transform: translateX(-50%) translateY(-6px) rotate(-2deg); }
...
export const Astronauta = styled.img<{ top: string; isColliding: boolean }>`
  position: absolute;
  left: 50%;
  top: ${({ top }) => top};
  ...
  animation: ${({ isColliding }) => isColliding
    ? css`${shake} 0.5s cubic-bezier(.36,.07,.19,.97) both`
...
export const SparksContainer = styled.div<{ top: number; left: number }>`
  position: absolute;
  top: ${({ top }) => top}px;
  left: ${({ left }) => left}px;
  width: 1px;
...
export const HudPlanetaImagem = styled.img<{ isCollected: boolean }>`
  height: 50px;
  opacity: ${({ isCollected }) => (isCollected ? 1 : 0.3)};
  filter: ${({ isCollected }) => (isCollected ? 'none' : 'grayscale(80%)')};
