...
export const IconeTrofeu = styled.div`
   /* CORREÇÃO PARA ALINHAR O TROFÉU NO CENTRO */
   display: flex;
   justify-content: center;
   align-items: center; 
...
interface BotaoVitoriaProps {
    $focado?: boolean;
}

export const BotaoVitoria = styled.button<BotaoVitoriaProps>`
   display: flex;
   align-items: center;
   justify-content: center;
   gap: 0.5rem;
   min-width: 180px;
   min-height: 60px;
   background-color: ${props => props.$focado ? '#FDBF5C' : '#f1f1f1'};
   color: ${props => props.$focado ? '#111' : '#4B27AF'};
   ...
   /* Estilo de foco manual (acessibilidade por piscada) */
   ${props => props.$focado && `
      transform: translate(3px, 3px);
      box-shadow: 3px 3px 0px #111;
      border-color: #FDBF5C; /* Adiciona destaque extra para o foco */
   `}
