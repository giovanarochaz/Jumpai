...
interface BotaoNavegacaoProps {
    $isFocusedManual?: boolean;
}

export const BotaoNavegacao = styled.button<BotaoNavegacaoProps>`
   display: flex;
   align-items: center;
   justify-content: center;
   gap: 0.5rem;
   ...
   /* Estilo de foco manual (acessibilidade por piscada no modo Manual) */
   ${({ $isFocusedManual }) => $isFocusedManual && css`
      background-color: ${cores.amarelo};
      border-color: ${cores.roxo};
      animation: ${focoPulsante} 1s infinite;
      transform: translate(3px, 3px);
      box-shadow: 3px 3px 0px ${cores.roxo};
   `}
   ...
   &:hover:not(:disabled) {
      ...
      animation: none; /* Desativa a animação de foco se o mouse for usado */
   }
