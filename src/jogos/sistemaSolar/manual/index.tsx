...
// (planetasInfo e tipos permanecem os mesmos)
const planetasInfo = [
...
const ManualSistemaSolar: React.FC<ManualSistemaSolarProps> = ({ aoIniciarMissao }) => {
  ...
  // <<< NOVO: Sistema de bloqueio único e centralizado
  ...
  // <<< NOVO: useEffect para o bloqueio inicial de 1 segundo ao abrir o manual
  useEffect(() => {
  ...
  // <<< REMOVIDO: Todos os useEffects antigos que escutavam 'estaPiscando' foram removidos
  // para serem substituídos pelo gerenciador central abaixo.

  // <<< NOVO: Gerenciador de Piscadas Centralizado
  useEffect(() => {
    ...
    if (tela !== 'configuracoes') {
      // --- Ação de Navegação ---
      avancarTela();
      // Define um novo bloqueio de 1 SEGUNDO
      ...
      // Define um novo bloqueio de 2 SEGUNDOS
      blockTimerRef.current = setTimeout(() => setBloquearPiscada(false), 500);
    }
  // Dependemos de `estaPiscando` para disparar, mas usamos outros estados para decidir o que fazer.
  // Incluí-los na lista de dependências garante que a função sempre use os valores mais recentes.
  }, [estaPiscando, mostrarCameraFlutuante, bloquearPiscada, tela, focoConfig, configuracoes]);


  // O useEffect para o ciclo automático das opções de configuração permanece o mesmo.
  useEffect(() => {
  ...
  const renderizarTela = () => {
    // O JSX dentro desta função não precisa de alterações
    // ...
    switch (tela) {
      case 'planetas':
        return (
